...
Reason: ${dynamicCommand.blockReason || 'Admin decision'}` });
        ...
        if (dynamicCommand.execute) {
            try {
                if (dynamicCommand.config.permissions === 'owner' && !jidManager.isOwner(msg)) { try { await sock.sendMessage(chatId, { text: '❌ *Owner Only Command*' }); } catch {} return; }
                if (commandName.includes('sticker')) await delay(1000);
                const currentBotName = settings.get('botname', BOT_NAME);
                await dynamicCommand.execute(sock, msg, args, currentPrefix, { OWNER_NUMBER: OWNER_CLEAN_NUMBER, OWNER_JID: OWNER_CLEAN_JID, OWNER_LID, BOT_NAME: currentBotName, VERSION, isOwner: () => jidManager.isOwner(msg), jidManager, store, statusDetector, updatePrefix: updatePrefixImmediately, getCurrentPrefix, rateLimiter, memberDetector, isPrefixless, commands, BOT_MODE });
            } catch (error) { UltraCleanLogger.error(`Command ${commandName} failed: ${error.message}`); }
        } else if (dynamicCommand.config && dynamicCommand.config.response) {
            try {
                if (dynamicCommand.config.permissions === 'owner' && !jidManager.isOwner(msg)) { 
                    await sock.sendMessage(chatId, { text: '❌ *Owner Only Command*' }); 
                    return; 
                }
                await sock.sendMessage(chatId, { text: dynamicCommand.config.response });
            } catch (error) { UltraCleanLogger.error(`Command ${commandName} failed: ${error.message}`); }
